---
name: fotingo
description: Use fotingo as the default workflow for starting issue work and submitting work for review.
license: MIT
compatibility: Requires fotingo CLI.
metadata:
  author: fotingo
  version: "1.12.0"
  generatedBy: "5.3.0-SNAPSHOT-e506507"
---

# Fotingo Workflow Skill

This skill configures Claude Code to use fotingo as the primary workflow interface for issue start, review submission, and supporting context operations.
...
```bash
fotingo start PROJ-123 --worktree-path .claude/worktrees -y --json
```
...
```bash
fotingo search reviewers ali --json
fotingo search assignees bob --json
fotingo search labels bug --json
```
...
```bash
fotingo review -y -r alice -r team/platform --assignee bob --labels bug
```

Fill the default Summary and Description sections with template overrides:

```bash
fotingo review -y --template-summary "Improve checkout decline handling" --template-description "Why: reduce support tickets from unclear payment errors.\n\nWhat changed:\n- clearer decline copy\n- structured telemetry\n- regression coverage"
```
...
```bash
fotingo review sync -y -r alice --remove-reviewers team/platform --assignee bob --remove-assignee carol --ready-for-review
```
